...
class Dashboard extends MY_Controller
{
	...
	public function index()
	{
		...
		$data['invest_approve'] = $this->mymodel->selectWhere('tbl_project_invest', array('investor_id' => $data['user']['id'], 'status_pembayaran' => 'APPROVE'));

		if ($data['user']) {
			$sum_harga = $this->mymodel->selectWithQuery("SELECT SUM(total_harga) as total_harga FROM tbl_project_invest WHERE investor_id = " . $data['user']['id'] . " AND status_pembayaran = 'APPROVE'");
			$count_project = $this->mymodel->selectWithQuery("SELECT count(id) as total_prj FROM tbl_project_invest WHERE investor_id = " . $data['user']['id'] . " AND status_pembayaran = 'APPROVE' GROUP BY project_id ");
	...
	public function editpassword()
	{
		...
		if ($_POST['newPassword'] != $_POST['confPassword']) {
			...
			if ($action == 'change') {
				$password = $this->mymodel->selectDataOne('tbl_investor', array('id' => $id));
