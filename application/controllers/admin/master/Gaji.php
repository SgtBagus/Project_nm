...
<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Gaji extends MY_Controller {
...

		{
			...
			$this->template->load('admin/template/template','admin/master/gaji/all-tbl_gaji',$data);
...

		{
			...
			$this->load->view('admin/master/gaji/add-tbl_gaji');
	...
	$this->form_validation->set_rules('dt[value]', '<strong>Value</strong>', 'required');
...

		{
	    	...
	    	if ($this->form_validation->run() == FALSE){
				...
				$str = $this->db->insert('tbl_gaji', $dt);
...

		{
	    	...
	    	if ($this->form_validation->run() == FALSE){
				...
				$id = $this->input->post('id', TRUE);		$dt = $_POST['dt'];
					...
					$this->mymodel->updateData('tbl_gaji', $dt , array('id'=>$id));
...

		{
			...
			$this->mymodel->updateData('tbl_gaji',array('status'=>$status),array('id'=>$id));

			redirect('admin/master/gaji');
